...
const BookingForm = ({ selectedService }: { selectedService?: string }) => {
  ...
  const onSubmit = (data: BookingFormData) => {
    ...
    const message = `*طلب حجز جديد من Dr. Fix*%0A%0A` +
      `*ماركة السيارة:* ${data.carMake}%0A` +
      `*موديل السيارة:* ${data.carModel}%0A` +
      `*سنة الصنع:* ${data.carYear}%0A` +
      `*نوع الخدمة:* ${serviceLabels[data.serviceType] || data.serviceType}%0A` +
      `*وصف المشكلة:* ${data.description}%0A` +
      `*رقم الجوال:* ${data.phone}%0A%0A` +
      (files.length > 0 ? `_ملاحظة: تم رفع ${files.length} ملفات توضيحية على الموقع_` : '');

    const whatsappUrl = `https://wa.me/966546870807?text=${message}`;
